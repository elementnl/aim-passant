...
module.exports = function registerDuelHandlers(io, socket) {
  socket.on('duel-player-state', (data) => {
    ...
    socket.to(room.code).emit('duel-opponent-state', data);
  });

  socket.on('duel-shoot', (data) => {
    const room = rooms.getByPlayer(socket.id);
    if (!room || room.state !== 'duel') return;
    socket.to(room.code).emit('duel-opponent-shoot', data);
  });

  socket.on('duel-hit', ({ damage }) => {
    const room = rooms.getByPlayer(socket.id);
    if (!room || room.state !== 'duel') return;
    socket.to(room.code).emit('duel-take-damage', { damage });
  });
  ...
  socket.on('duel-result', ({ winner }) => {
    ...
    if (result.kingCaptured) {
      ...
      io.to(room.code).emit('duel-end', { winner, state: result.state });
      io.to(room.code).emit('game-over', { reason: 'king-captured', winner: winnerColor });
    } else {
      io.to(room.code).emit('duel-end', { winner, state: result.state });
    }
  });
